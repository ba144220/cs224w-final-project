...
OUTPUT_PATH = "designs/0/circuit_graph.png"
...
def build_node_inventory(
    module: Dict, nodes: List[Tuple[str, str]], name2idx: Dict[str, int]
) -> None:
    """
...
def print_summary(nodes: List[Tuple[str, str]], edges: set, data: Data) -> None:
    """
...
def visualize_graph(nodes: List[Tuple[str, str]], data: Data, output_path: str) -> None:
    """
    ...
    labels = {i: f"{name}\n({node_type})" for i, (name, node_type) in enumerate(nodes)}

    # Map node colors based on type
    node_colors = [COLOR_MAP.get(node_type, "#D3D3D3") for _, node_type in nodes]
    ...
    nx.draw_networkx_nodes(
        graph_to_draw, pos, node_color=node_colors, node_size=3000, alpha=0.9, ax=ax
    )
    ...
    nx.draw_networkx_labels(
        graph_to_draw, pos, labels, font_size=9, font_weight="bold", ax=ax
    )
    ...
    ax.set_title("Circuit Graph - Hierarchical Layout", fontsize=16, fontweight="bold")
    ax.axis("off")
    plt.tight_layout()
...
def convert_net_to_graph(
    ...
    # Visualize graph
    visualize_graph(nodes, data, output_image_path)
